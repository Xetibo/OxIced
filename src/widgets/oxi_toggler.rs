    ...
    fn update(
        ...
        match event {
            ...
            | Event::Touch(touch::Event::FingerPressed { .. }) => {
                ...
                if mouse_over {
                    let state = tree.state.downcast_mut::<State<Renderer::Paragraph>>();
                    state.transition.go_mut(!self.is_toggled, Instant::now());
        ...
        let animation_progress = state.transition.interpolate(0.0, 1.0, Instant::now());
...
// }
//
// pub fn toggler<'a, M>(is_checked: bool) -> OxiToggler<'a, M> {
